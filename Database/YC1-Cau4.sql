...
GRANT SELECT, INSERT, UPDATE, DELETE ON VPD.DANGKY TO Role_SV;
GRANT SELECT ON VPD.DANGKY TO Role_GV;
GRANT SELECT ON VPD.MOMON TO Role_GV;
GRANT SELECT ON VPD.HOCPHAN TO Role_GV;
GRANT SELECT, INSERT, UPDATE, DELETE ON VPD.DANGKY TO Role_NV_PDT;
GRANT SELECT, UPDATE (DIEMTH, DIEMQT, DIEMCK, DIEMTK) ON VPD.DANGKY TO Role_NV_PKT;
GRANT SELECT ON VPD.NHANVIEN TO Role_NV_PKT;
...
/
BEGIN
   EXECUTE IMMEDIATE 'CREATE USER NV0715 IDENTIFIED BY 123';
EXCEPTION
   WHEN OTHERS THEN NULL;
END;
/
...
END;
/
GRANT CREATE SESSION TO NV0715, SV0001, NV0612, NV0742;

-- Gán role cho user
GRANT Role_SV TO SV0001;
GRANT Role_NV_PDT TO NV0715;
...
BEFORE INSERT OR UPDATE OR DELETE ON VPD.DANGKY
FOR EACH ROW
DECLARE
    v_user VARCHAR2(30) := SYS_CONTEXT('USERENV', 'SESSION_USER');
...
-- Tạo hàm chính sách VPD.DANGKY_POLICY_FUNC
CREATE OR REPLACE FUNCTION VPD.DANGKY_POLICY_FUNC (
...
-- Kiểm tra
SELECT * FROM VPD.DANGKY;

INSERT INTO VPD.DANGKY (MASV, MAMM)
VALUES ('SV0001', 'MM002');

UPDATE VPD.DANGKY
SET MAMM = 'MM003'
WHERE MASV = 'SV0001' AND MAMM = 'MM003';
DELETE FROM VPD.DANGKY
